...
|---------|------|-----------|---------|
| **v3.1.2** | 2026-07-05 | executor, app | Fix "Unknown error" on huge exception messages: `error_message` is now capped at source (4K, head+tail preserved) and `error_traceback` capped at 20K tail-first in `executor.py` — previously a giant exception message (e.g. `raise ValueError(huge_string)`) inflated the payload until `enforce_response_budget()` replaced the whole response dict, and MCP clients saw `Execution Error: Unknown error` with the real message lost. Also: `app/__init__.py` now re-exports `__version__` from `app/version.py` (single source of truth — `/health` previously hardcoded 1.0.0), and stale duplicate `app/app/version.py` (2.9.1, referenced nowhere) removed |
| **v3.1.1** | 2026-07-05 | response_guard | Tail-aware truncation for tracebacks (PR #13): `smart_truncate()` now detects Python tracebacks / `Execution Error: ...\n\nTraceback:` blocks and keeps the END of the text (exception type + message) instead of head-first truncation, which silently discarded exactly the line callers need. Keeps a short head anchor (the error summary line) when it fits. JSON and plain-text truncation strategies unchanged. Adds `tests/test_response_guard.py` (8 tests, first test infrastructure in the repo) |
